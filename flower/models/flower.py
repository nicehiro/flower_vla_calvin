...
class FLOWERVLA(pl.LightningModule):
    ...
    def _load_pretrained_weights(
        self, pretrained_model_path: str, mean_resizing: bool = False
    ):
        ...
        if pretrained_model_path.endswith(".safetensors"):
            ...
            }  # Create checkpoint-like structure for compatibility
            ...
            for param_name, original_param in original_state_dict.items():
                if ema_idx < len(ema_weights_list):
                    ...
                    if ema_weight.shape == original_param.shape:
                        ...
                        found_match = False
                        for temp_idx in range(
                            ema_idx, min(ema_idx + 20, len(ema_weights_list))
                        ):
                        ...
                        if not found_match:
                            ...
                            print(
                                f"Warning: No matching EMA weight found for {param_name}, using original"
                            )
                            state_dict[param_name] = original_param
                else:
                    # No more EMA weights available, use original
                    print(
                        f"Warning: Ran out of EMA weights at {param_name}, using original"
                    )
                    state_dict[param_name] = original_param

            print(
                f"Successfully matched {ema_idx} EMA weights out of {len(ema_weights_list)} total"
            )
        ...
        if missing_keys:
            print(
                f"  ⚠️ Missing keys (not found in checkpoint, using default init): {len(missing_keys)}"
            )
    ...
    def _init_flags(self, **kwargs):
        ...
        if self.vlm_prompt_style not in [
            "default",
            "feature_focused",
            "state_oriented",
        ]:
    ...
    def _init_dimensions(self, **kwargs):
        ...
        if self.dit_dim % self.n_heads != 0:
            raise ValueError(
                f"dit_dim ({self.dit_dim}) must be divisible by n_heads ({self.n_heads})"
            )

    def _setup_vlm(
        self, vlm_path: str, freeze_vision_tower: bool, freeze_florence: bool
    ):
    ...
    def _setup_dit_components(self, **kwargs):
        ...
        if not use_rope and not use_nope:
            self.positional_encoding = nn.Parameter(
                torch.randn(1, kwargs["act_window_size"], dit_dim) * 0.1
            )
        ...
        for action_name, action_idx in self.action_space_index.action_spaces.items():
            ...
            if self.use_proprio:
                ...
                self.proprio_encoders[action_name] = (
                    Mlp(input_dim, dit_dim, out_features=dit_dim, drop=0.2).to(
                        self.device
                    )
    ...
    def configure_optimizers(self):
        ...
        scheduler = TriStageLRScheduler(
            optimizer, OmegaConf.create(self.lr_scheduler_config)
        )
    ...
    def validation_step(
        self, batch: Dict[str, Dict], batch_idx: int
        ...
        with torch.no_grad():
            ...
            # Sample actions
            action_pred = self.sample_actions(
                noise_actions, obs_features, inference=True
            )
    ...
    def dit_forward(
        ...
        if self.use_proprio_history and cond_dict.get("proprio_history") is not None:
            ...
            proprio_history = cond_dict["proprio_history"].to(
                default_dtype
            )  # [B, history_len, state_dim]
            ...
            proprio_embeds = self.encode_proprio(
                proprio, action_type, frequency_embeds.shape
            )
        else:
    ...
    def encode_proprio(
        ...
        encoded_proprio = torch.zeros(
            batch_size, self.dit_dim, device=self.device, dtype=default_dtype
        )

        for action_name, action_idx in self.action_space_index.action_spaces.items():
            mask = action_type == action_idx
            if mask.any():
                encoded_proprio[mask] = self.proprio_encoders[action_name](
                    proprio[mask]
                ).squeeze(1)
    ...
    def encode_observations(self, batch: Dict) -> torch.Tensor:
        ...
        merged_embeds = torch.cat(
            [image_features, task_prompt, text_embeds.to(image_features.device)], dim=1
        )
        ...
        features = self.vlm.get_encoder()(
            inputs_embeds=merged_embeds, attention_mask=attention_mask
        ).last_hidden_state
        ...
        if self.use_proprio_history and "robot_obs" in batch:
            ...
            proprio_history = (
                batch["robot_obs"].to(device).to(default_type)
            )  # [B, history_len, state_dim]
        elif self.use_proprio and "robot_obs" in batch:
            # Legacy: single frame
            proprio = (
                batch["robot_obs"][:, -1, :].to(device).to(default_type)
            )  # [B, state_dim]
        ...
        return {
            ...
            "action_type": torch.ones_like(
                action_type_tensor
            ),  # actiont ype is always 1
            "proprio": proprio,
    ...
    def encode_actions(
        ...
        encoded = torch.zeros(
            batch_size, z.shape[1], self.dit_dim, device=self.device
        ).to(default_dtype)
    ...
    def decode_actions(
        ...
        decoded = torch.zeros(
            batch_size, z.shape[1], max_action_dim, device=self.device
        ).to(default_dtype)
    ...
    def print_encoded_texts(self, batch, device):
        ...
        input_ids = (
            batch[self.goal_modalities][self.lang_modalities[0]]["input_ids"][0]
            .squeeze(0)
            .to(self.device)
        )
        ...
        decoded_texts = self.processor.tokenizer.batch_decode(
            text_embeds.cpu(), skip_special_tokens=True
        )
        print("Encoded texts:")
    ...
    def _log_validation_metrics(self, pred_loss, val_total_act_loss_pp):
        ...
        self.log(
            f"val_act/{self.modality_scope}_act_loss_pp", pred_loss, sync_dist=True
        )
        ...
        self.log(
            "val_act/action_loss", val_total_act_loss_pp / n_modalities, sync_dist=True
        )

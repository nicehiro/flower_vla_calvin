...
class ProprioHistoryTokenizer(nn.Module):
    """Projects proprio history frames into cross-attention tokens.

    Each frame of robot state is projected to the DiT dimension, allowing
    action tokens to attend to individual state history frames via cross-attention.
    This enables the model to learn which historical states are relevant for
    each predicted action.
    """
    ...
    def __init__(self, state_dim: int, output_dim: int):
        ...
        """
        super().__init__()
        self.proj = nn.Linear(state_dim, output_dim)
    ...
    def forward(self, state_history: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            tokens: [B, history_len, output_dim] tensor for cross-attention
        """
        return self.proj(state_history)  # [B, 5, 15] → [B, 5, 1024]
...
class FLOWERVLA(pl.LightningModule):
    def __init__(
        ...
        # Proprio history buffer for inference (maintains last proprio_history_len observations)
    ...
    def _setup_dit_components(self, **kwargs):
        ...
        # Proprio history tokenizer (cross-attention approach)
        if self.use_proprio_history:
            self.proprio_tokenizer = ProprioHistoryTokenizer(
                state_dim=self.proprio_state_dim,
                output_dim=dit_dim,
            )
            logger.info(f"[ProprioHistory] Tokenizer created: state_dim={self.proprio_state_dim}, output_dim={dit_dim}")
    ...
    def dit_forward(self, z: torch.Tensor, t: torch.Tensor, cond_dict: dict) -> torch.Tensor:
        ...
        # Setup context - concatenate proprio tokens if available
        cx = z
        if self.use_cross_attn:
            if proprio_tokens is not None:
                # Concatenate proprio tokens to VLM features for cross-attention
                context = torch.cat([cond, proprio_tokens], dim=1)  # [B, 654+5, 1024]
                if not hasattr(self, '_context_shape_logged'):
                    logger.info(f"[ProprioHistory] Context shape: {context.shape} (VLM: {cond.shape[1]} + proprio: {proprio_tokens.shape[1]})")
                    self._context_shape_logged = True
            else:
                context = cond
        else:
            context = None
    ...
    def reset(self):
        ...
        self.rollout_step_counter = 0
        self.pred_action_seq = None
        # Reset proprio history buffer for new rollout
        self.proprio_history_buffer = None
        self.eval()

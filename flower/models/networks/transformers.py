    ...
    def __init__(
        ...
        if use_rope:
            q_cos, q_sin = precompute_freqs_1d(
                self.head_dim, query_seq_len, theta=rope_theta
            )
            k_cos, k_sin = precompute_freqs_1d(
                self.head_dim, context_seq_len, theta=context_rope_theta
            )
...
class FlowBlock(nn.Module):
    ...
    def forward(
        ...
        x_self = self.self_attn(
            x_mod, custom_attn_mask=custom_attn_mask, is_causal=is_causal
        )
        ...
        if self.use_cross_attn:
            ...
            x_cross = self.cross_attn(
                x_norm, context, custom_attn_mask=custom_cross_attn_mask
            )
            x_out = x_out + x_cross
